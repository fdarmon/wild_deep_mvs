...
class network(nn.Module):
    ...
    def forward(self, ref_img, src_imgs, ref_in, src_in, ref_ex, src_ex, \
        ...
        ## Feature extraction
        ref_feature_pyramid = self.featurePyramid(ref_img, nscale)

        src_feature_pyramids = []
        for i in range(nsrc):
            src_feature_pyramids.append(self.featurePyramid(src_imgs[i], nscale))
        ...
        depth_hypos = calSweepingDepthHypo(ref_in_multiscales[:, -1], src_in_multiscales[:, 0, -1], ref_ex, src_ex,
                                           depth_min, depth_max, nhypothesis_init=48 if self.training else 96)

        ref_volume = ref_feature_pyramid[-1].unsqueeze(2).repeat(1, 1, len(depth_hypos[0]), 1, 1)

        volume_sum = ref_volume
        # modifided from original github, no inline
        ...
        for src_idx in range(nsrc):
            ...
            warped_volume = homo_warping(src_feature_pyramids[src_idx][-1], ref_in_multiscales[:, -1],
                                         ...
                                         depth_hypos, ref_shape = volume_sum.shape[3:])
        ...
        for id_level, level in enumerate(range(nscale - 2, -1, -1)):
            ...
            # computation in calDepthHypo unstable on some testing data

            if self.training:
                # halve the interval size at each level
                interval_size = (depth_max - depth_min) / 48 / 2 ** (id_level + 1)
                depth_hypos = torch.stack([
                    depth_up + i * interval_size.view(-1, 1, 1) for i in range(-4, 4)
                ], dim=1)

            else:
                depth_hypos = calDepthHypo(depth_up, ref_in_multiscales[:, level, :, :],
                                           src_in_multiscales[:, :, level, :, :], ref_ex, src_ex, depth_min,
                                           depth_max, level)
                if torch.isnan(depth_hypos).any():
                    print("NAN")

            # MODIFICATION FROM ORIGINAL GITHUB REPO
            cost_volume = proj_cost(nsrc, ref_feature_pyramid[level], src_feature_pyramids, level,
