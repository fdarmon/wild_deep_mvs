...
from .nn_utils import ListModule, UNet, multi_dims, CSPN, soft_argmin, entropy, StackCGRU, hourglass, bin_op_reduce, \
    groupwise_correlation
from .preproc import scale_camera, recursive_apply
from .homography import get_pixel_grids, get_homographies, homography_warping, interpolate
...
class SingleStage(nn.Module):
    ...
    def __init__(self):
        ...
        self.uncert_net = UncertNet(1)  # MVS
    ...
    def build_cost_volume(self, ref, ref_cam, src, src_cam, depth_num, depth_start, depth_interval, s_scale, d_scale):
        ...
        warped_src_nd_c_h_w = homography_warping(src_nd_c_h_w, Hs.view(-1, *Hs.size()[2:]), ref_shape)  # n*d chw
        warped_src = warped_src_nd_c_h_w.view(-1, depth_num // d_scale, c, *ref_shape).transpose(1, 2)  # ncdhw
        return warped_src
        return warped_src
