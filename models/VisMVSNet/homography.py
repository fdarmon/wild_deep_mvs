...
def get_homographies(left_cam, right_cam, depth_num, depth_start, depth_interval, inv=False):
    ...
    with torch.no_grad():
        ...
        # depth nd1111/ndhw11
        if not inv:
            depth = depth_start + depth_interval * torch.arange(0, depth_num, dtype=left_cam.dtype,
                                                                device=left_cam.device).view(1, d, 1, 1)
        else:
            depth_end = depth_start + (depth_num - 1) * depth_interval
            inv_interv = (1 / (depth_start + 1e-9) - 1 / (depth_end + 1e-9)) / (depth_num - 1 + 1e-9)
            depth = 1 / (1 / (depth_end + 1e-9) + inv_interv * torch.arange(0, depth_num, dtype=left_cam.dtype,
                                                                            device=left_cam.device).view(1, d, 1, 1))
    ...
    if torch.isnan(homographies).any():
        raise Exception("Nan")
...
def interpolate(image, coord, norm_size=None):  # nchw, nhw2 => nchw
    if norm_size is None:
        norm_size = coord.shape[2:]
    ...
    with torch.no_grad():
        ...
        warped_coord[..., 0] /= norm_size[1]
        warped_coord[..., 1] /= norm_size[0]
        warped_coord = (warped_coord * 2 - 1).clamp(-1.1, 1.1)

    if torch.isnan(image).any():
        raise Exception("coords have nan")

    warped = torch.nn.functional.grid_sample(image, warped_coord, mode='bilinear', padding_mode='zeros',
                                             align_corners=True)

    return warped


def homography_warping(input, H, ref_shape=None):  # nchw, n33/nhw33 -> nchw
    if len(H.size()) == 3: H = H.view(-1, 1, 1, 3, 3)
    if ref_shape is None: ref_shape = input.shape[2:]
    with torch.no_grad():
        pixel_grids = get_pixel_grids(*ref_shape).unsqueeze(0)  # 1hw31
        ...
        valids = warped_homo_coord[..., 2] > 0  # nhw
        warped_coord = warped_homo_coord[..., :2] / warped_homo_coord[..., 2:3].clamp(min=1e-9)  # nhw2
        warped_coord[~valids, :] = -10
        # MODIFICATION FROM ORIGINAL GITHUB REPO

    warped = interpolate(input, warped_coord, norm_size=input.shape[2:])
    return warped  # nchw